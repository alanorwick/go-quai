...
func (p *StateProcessor) Process(block *types.WorkObject, batch ethdb.Batch) (types.Receipts, []*types.Transaction, []*types.Log, *state.StateDB, uint64, uint64, *multiset.MultiSet, error) {
	...
	RedeemLockedQuai(p.hc, statedb, block.Number(nodeCtx), lockupContractAddress)
	...
	for i, tx := range block.Transactions() {
		...
		if tx.Type() == types.QiTxType {
			...
			qiTxFee, fees, etxs, err, timing := ProcessQiTx(tx, p.hc, checkSig, firstQiTx, header, batch, p.hc.headerDb, gp, usedGas, p.hc.pool.signer, p.hc.NodeLocation(), *p.config.ChainID, &etxRLimit, &etxPLimit, utxosCreatedDeleted)
			if err != nil {
		...
		if tx.Type() == types.ExternalTxType {
			...
			if etx.To().IsInQiLedgerScope() {
				if etx.ETXSender().Location().Equal(*etx.To().Location()) { // Quai->Qi Conversion
					...
					value := etx.Value() // convert Quai to Qi
					txGas := etx.Gas()
	...
	if block.ExchangeRate().Cmp(exchangeRate) != 0 {
		return nil, nil, nil, nil, 0, 0, nil, fmt.Errorf("invalid exchange rate used (remote: %d local: %d)", block.ExchangeRate(), exchangeRate)
	}
	...
	log.Global.Error("STATE PROCESSOR: etx values on top of block", parent.NumberArray(), block.OutboundEtxHash())
	for _, etx := range updatedEmittedEtxs {
		if types.IsConversionTx(etx) {
			log.Global.Error("tx value for conversion", etx.Value())
		}
	}
	...
	return receipts, updatedEmittedEtxs, allLogs, statedb, *usedGas, *usedState, multiSet, nil
}
...
func RedeemLockedQuai(hc *HeaderChain, statedb *state.StateDB, currentBlockHeight *big.Int, lockupContractAddress common.Address) error {
	// Array of specific block depths for which we will redeem the Quai
	blockDepths := []uint64{
		params.ConversionLockPeriod,      // Minimum lockup period
		params.LockupByteToBlockDepth[0], // 2 hours
		params.LockupByteToBlockDepth[1], // 4 hours
	...
	for _, blockDepth := range blockDepths {
		...
		targetBlockHeight := new(big.Int).Sub(currentBlockHeight, new(big.Int).SetUint64(blockDepth))

		// Fetch the header and block at the calculated target height
		targetHeader := hc.GetHeaderByNumber(targetBlockHeight.Uint64())
		if targetHeader == nil {
		...
		for _, etx := range targetBlock.Body().ExternalTransactions() {
			...
			if types.IsCoinBaseTx(etx) && etx.ETXSender().IsInQuaiLedgerScope() {
				...
				_, _, err := vm.RedeemQuai(statedb, *sender, gasPool, currentBlockHeight, lockupContractAddress)
				if err != nil {
			...
			if types.IsConversionTx(etx) {
				...
				if etx.ETXSender().IsInQiLedgerScope() {
					receiver := etx.To()
					gasPool := new(types.GasPool).AddGas(math.MaxUint64) // Use unlimited gas, as this is part of block processing
					_, _, err := vm.RedeemQuai(statedb, *receiver, gasPool, currentBlockHeight, lockupContractAddress)
					if err != nil {
						return fmt.Errorf("failed to redeem Quai for coinbase address at block depth %d: %w", blockDepth, err)
					}
				}
