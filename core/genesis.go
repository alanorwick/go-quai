...
func SetupGenesisBlockWithOverride(db ethdb.Database, genesis *Genesis, nodeLocation common.Location, startingExpansionNumber uint64, logger *log.Logger) (*params.ChainConfig, common.Hash, error) {
	...
	if _, err := state.New(header.EVMRoot(), header.UTXORoot(), state.NewDatabaseWithConfig(db, nil), state.NewDatabaseWithConfig(db, nil), state.NewDatabaseWithConfig(db, nil), nil, nodeLocation); err != nil {
		if genesis == nil {
			genesis = DefaultGenesisBlock()
		}
...
// ToHeader creates the genesis header
func (g *Genesis) ToHeader(startingExpansionNumber uint64) *types.Header {
...
func (g *Genesis) ToBlock(db ethdb.Database, startingExpansionNumber uint64) *types.Block {
	head := g.ToHeader(startingExpansionNumber)
